...
import com.mysite.manage.file.FtpClientUtil;
...
public class MainController {
    ...
    public ResponseEntity<Submission> getResult(@RequestParam String username, @RequestParam String password, @RequestParam int id) {
        return new ResponseEntity<>(submissionService.checkSubmission(username, password, id), HttpStatus.OK);
    ...
    @GetMapping("/new")
    public ResponseEntity<Integer> getSubmission(@RequestParam String host,
                              @RequestParam int port,
                              @RequestParam String username,
                              @RequestParam String password){
        Integer earliest = submissionService.getEarliest();
        try{
            FtpClientUtil ftpClientUtil = new FtpClientUtil(host, port, username, password);
            ftpClientUtil.uploadFile("/", "code/"+earliest+".py");
            ftpClientUtil.disconnect();
        } catch (IOException e){
            e.printStackTrace();
        }
        return new ResponseEntity<>(earliest, HttpStatus.OK);
    }
